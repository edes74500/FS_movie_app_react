...
export const sortGenreList = () => {
  return (dispatch, getState) => {
    const { genreList } = getState();
    // const sortedGenreList = [...genreList]; // Créez une copie de la liste pour éviter de modifier l'original
    const sortedGenreList = genreList..slice().sort((a, b) => b.selected - a.selected); // Triez la liste copiée
...
export const updateGenreList = (e) => {
  return (dispatch, getState) => {
    ...
    const updatedDataGenres = genreList.map((genre) => {
...
export const resetGenreList = () => {
  return (dispatch, getState) => {
    ...
    const resetData = genreList.map((genre) => ({ ...genre, selected: false }));
