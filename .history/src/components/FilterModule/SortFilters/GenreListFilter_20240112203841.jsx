...
const StyledContentWrapper = styled.div`
  ...
  &.is-open {
    ...
    height: ${({ filters }) => (filters ? filters.length * variables.filterItemHeight : 0)}px;
...
const GenreListFilter = ({ data, filters }) => {
  ...
  let [displayedGenre, setDisplayedGenre] = useState([]);
  ...
  return (
    <SortContainer filterName="Genre" icon={userHasSelectedGenres ? MdOutlineCancel : ""} onClick={handleResetFilter} onClickExpand={handleExpand}>
      <StyledContentWrapper ref={expandbleRef} className="content-wrapper" filters={filters}>
        {data.map((category, index) => {
          const displayedGenreV = filters.find((genre) => genre.toLowerCase() === category.name.toLowerCase());
          if (!displayedGenreV) {
            return null;
          }

          setDisplayedGenre((old) => [...old, displayedGenreV]);
