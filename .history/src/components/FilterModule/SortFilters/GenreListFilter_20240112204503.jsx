...
const StyledContentWrapper = styled.div`
  ...
  &.is-open {
    ...
    height: ${({ displayedGenres }) => (displayedGenres ? displayedGenres.length * variables.filterItemHeight : 0)}px;
...
const GenreListFilter = ({ data, filters }) => {
  ...
  return (
    <SortContainer filterName="Genre" icon={userHasSelectedGenres ? MdOutlineCancel : ""} onClick={handleResetFilter} onClickExpand={handleExpand}>
      <StyledContentWrapper ref={expandbleRef} className="content-wrapper" displayedGenres={displayedGenres}>
        {data.map((category, index) => {
          const displayedGenreV = if (filters.find((genre) => genre.toLowerCase() === category.name.toLowerCase())) return category.id;
          if (!displayedGenreV) {
            return null;
          }

          displayedGenres.push(displayedGenreV);
