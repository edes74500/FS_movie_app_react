...
const GenreListFilter = ({ data, filters }) => {
  ...
  const handleToogleFiler = (e) => {
    dataGenres.forEach((genre) => {
      if (genre.id === Number(e.currentTarget.id)) {
        genre.displayed = !genre.displayed;
      } else {
        genre.displayed = genre.displayed;
      }
      console.log(typeof Nunber(e.currentTarget.id));
    });
  };
  ...
  useEffect(() => {
    const newData = data
      .map((genre) => ({ ...genre, selected: false, displayed: shownGenresFilm.includes(genre.name) ? true : false }))
  ...
  return (
    ...
    <SortContainer filterName="Genre" icon={userHasSelectedGenres ? MdOutlineCancel : ""} onClick={handleResetFilter} onClickExpand={handleExpand}>
      <StyledContentWrapper ref={expandbleRef} className="content-wrapper" dataGenres={dataGenres}>
