...
const StyledContentWrapper = styled.div`
  ...
  height: ${({ $dataGenres, $genreListIsOpened }) =>
    $dataGenres.filter((genre) => genre.selected == true).length >= 5 && !$genreListIsOpened
      ? $dataGenres.filter((genre) => genre.selected == true).length * "var(--filter-item-height)"
      : $genreListIsOpened
      ? $dataGenres.filter((genre) => genre.displayed == true).length * "var(--filter-item-height)"
      : 5 * "var(--filter-item-height)"}px;
  transition: 1s;
  overflow: hidden;
