...
const StyledContentWrapper = styled.div`
  ...
  &.is-open {
    height: ${({ $dataGenres }) =>
      $dataGenres ? $dataGenres.filter((genre) => genre.displayed == true).length * variables.filterItemHeight : 0}px;
  }
`;
...
//   const dataGenres = useSelector((state) => state.genreList); // recupere les genres de la base de données

//   //A METTRE DES LA REQUETE API
//   useEffect(() => {
//     const newData = data.map((genre) => ({
//       ...genre,
//       selected: false,
//       displayed: shownGenresFilm.includes(genre.name) ? true : false,
//     }));
//     setdataGenres(newData);
//     console.log(dataGenresdeux);
//   }, [data]);
...
//       }
//     });
//     if (!genreListIsOpened && updatedDataGenres.filter((genre) => genre.selected).length >= 5) {
