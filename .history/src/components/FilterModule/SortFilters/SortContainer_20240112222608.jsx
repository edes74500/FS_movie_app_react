...
const SortContainer = ({ filterName, children, icon: Icon, onClick, onClickExpand, moreOptions }) => {
  ...
  return (
    <StyledGenreListFilter data-indentifier="GenreListFilter">
      <div className="filter-title-container" onClick={onClickExpand}>
        <h3>
          <TbCategoryPlus />
          Par {filterName}
        </h3>
        <span className="dropdown-icon">{Icon && <Icon onClick={onClick} />}</span>
      </div>
      <div className="filter-content-container">{children}</div>
      {moreOptions && <div className="more-content">+ d'options</div>}
